...
export default function TrustBar() {
  ...
  return (
    <div
      ref={ref}
      className="bg-white border-y border-black/[0.08]"
    >
      <div className="max-w-[1200px] mx-auto px-6 lg:px-10 py-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-0 lg:divide-x lg:divide-black/[0.06]">
          ...
          />
          <StatItem
            value={`${fmt(operations)}+`}
            label="Operaciones registradas"
            inView={inView}
          />
          <div
            className={`text-center transition-all duration-500 delay-200 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
          >
            <div className="font-mono text-[22px] font-semibold text-text tracking-[-0.02em]">
              Claude
            </div>
            <div className="font-mono text-[10px] text-text-muted uppercase tracking-[0.12em] mt-1">
              Motor de IA
          ...
          <div
            className={`text-center transition-all duration-500 delay-300 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}
          >
            <div className="font-mono text-[22px] font-semibold text-text tracking-[-0.02em]">
              4,9★
            </div>
