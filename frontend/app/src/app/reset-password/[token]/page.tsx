...
import { AlertCircle, CheckCircle, XCircle, Eye, EyeOff, Sun, Moon } from "lucide-react";
...
function passwordStrength(pw: string): { score: number; color: string } {
  ...
  return { score, color: colors[score] ?? "#d94040" };
}
...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/auth/validate-reset-token/${token}/`, {
      credentials: "include",
    })
      .then((res) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(body?.detail ?? t("errorGeneral"));
      setFormState("idle");
    } catch {
        ...
        <div className="bg-surface border border-white/[0.08] p-6">
          ...
          {tokenState === "invalid" && (
            <div className="text-center py-2">
              <XCircle size={32} className="text-loss mx-auto mb-4" />
              ...
              <Link
                ...
                className="font-sans text-[13px] text-green hover:underline"
              >
                {t("requestNewLink")}
              </Link>
            </div>
          ...
          {tokenState === "valid" && formState !== "success" && (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-3" noValidate>
                ...
                <ul className="space-y-[5px] pt-1">
                  {[
                    { ok: hasMin8, label: t("req8chars") },
                    { ok: hasUpper, label: t("reqUppercase") },
                    { ok: hasNumber, label: t("reqNumber") },
                  ].map(({ ok, label }) => (
                    <li key={label} className="flex items-center gap-2">
                      <div
                        className="w-[5px] h-[5px] rounded-full flex-shrink-0"
                        style={{ backgroundColor: ok ? "#2fac66" : "rgba(255,255,255,0.15)" }}
                      />
                      <span
                        className="font-mono text-[11px] transition-colors"
                        style={{ color: ok ? "#2fac66" : "var(--text-muted)" }}
                      >
                        {label}
                      </span>
                    </li>
                ...
                <button
                  type="submit"
                  disabled={formState === "loading" || !password || !passwordConfirm}
